...
contract TrustManager {
     ...
     * @dev 更新节点行为指标
     * @param _node 节点地址
     * @param _successRate 交易成功率 (0-100)
     * @param _responseTime 响应时间(ms)
     * @param _onlineTime 在线时长增量(秒)
     */
    ...
    function _calculateAndUpdateTrustValue(address _node) private {
        ...
        uint calculatedTrust = (
            weightAlpha * normalizedSuccessRate +
            weightBeta * normalizedResponseInverse +
            weightGamma * normalizedOnlineTime
        ) / 10000;
        uint risk = nodeRiskExposure[_node];
        if (risk > 100) risk = 100;
        uint riskPenalty = (weightTheta * risk) / 10000;
        uint adjusted = calculatedTrust > riskPenalty ? calculatedTrust - riskPenalty : 0;
        uint newTrustValue = (adjusted * MAX_TRUST_VALUE) / 100;
    ...
    /**
     * @dev 应用邻域推荐机制
     * 公式: T_final = λ * T_self + (1-λ) * Σ(w_i * T_i) / Σw_i
     * @param _node 节点地址
     * @param _selfTrust 节点自身计算的信任值
     * @return 融合后的信任值
     */
    ...
    function _recomputeRelTrust(address _node, uint8 _rel) internal {
        ...
        uint maxSec = onlineMaxSeconds;
        uint not = st.onlineTime > maxSec ? 100 : (st.onlineTime * 100) / maxSec;
        ...
        uint finalTrust;
        if (totalWeight == 0) finalTrust = selfTrust;
        else {
            uint neighTrust = weightedSum / totalWeight;
